...
Begin Window WindowPayment
   ...
   Begin CustomCanvas_Background CustomCanvas_Background1
      ...
      Begin TextField1 FieldChange
         ...
         Width           =   100
      End
      Begin TextField1 FieldTotalAmount
         AllowAutoDeactivate=   True
         AllowFocusRing  =   True
         AllowSpellChecking=   False
         AllowTabs       =   False
...
#tag Events ButtonSave
	...
	#tag Event
		Sub LeftClick()
		  ...
		  data.Value("transaction_id") = mLastInsertedID
		  data.Value("total_amount") = EncodeURLComponent(FieldTotalAmount.Value)
		  data.Value("amount_tendered") = EncodeURLComponent(FieldAmountTendered.Value)
		  data.Value("change") = EncodeURLComponent(FieldChange.Value)
