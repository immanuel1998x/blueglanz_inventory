#tag Class
Protected Class CustomSearchField
Inherits TextField
	#tag Event
		Sub GotFocus()
		  isFocused = True
	...
	#tag Method, Flags = &h21
		Private Sub drawIcon(g as Graphics)
		  If icon <> Nil Then
		    ...
		    If Not (isFocused Or isHovered) Then
		      g.Transparency = 50
		    End If
		    
		    Const margin = 5
		    Dim iconSize As Double = Me.Height
		    Dim w,h As Double
	...
	#tag Method, Flags = &h21
		Private Function isWithinIconArea(X as Integer, Y as Integer) As Boolean
		  If iconArea.LastRowIndex = -1 Then Return False
		  ...
		  Dim iconRight As Integer = iconArea(0) + iconArea(2)
		  Dim iconBottom As Integer = iconArea(1) + iconArea(3)
	...
	#tag Method, Flags = &h0
		Sub searchAction()
		  // Support funtion for raising "Action" event
		  
		  RaiseEvent Pressed(Value)
		End Sub
	#tag EndMethod
	...
	#tag Hook, Flags = &h0, Description = 55736572206861732070726573736564205B63617272696167652072657475726E5D
		Event Pressed(searchString as String)
	#tag EndHook
	...
	#tag Constant, Name = kIdleColor, Type = Color, Dynamic = False, Default = \"&cDDDFE3", Scope = Private
		#Tag Instance, Platform = Any, Language = Default, Definition  = \"&cDDDFE3"
	#tag EndConstant
